...
export async function runBuild(idea) {
    if (!idea.trim()) {
...
export async function runDepartment(dept, agent) {
    if (!dept) {
...
async function executeAgent(client, profile, agentPath, dept) {
...
async function populateProfile(client, idea) {
  ...
  "launchTarget": "6 months from today"
    ...
    try {
        ...
        for (const [key, value] of Object.entries(profile)) {
            ...
            claudeContent = claudeContent.replace(new RegExp(`(- ${label}:\\s*)\\[.*?\\]`), `$1${value}`);
        }
...
function gatherContext(agentContent, currentDept) {
...
function updateBuildLog(dept, agent, failed = false) {
    const claudePath = join(USER_CWD, 'CLAUDE.md');
    if (!existsSync(claudePath))
        return;
    ...
    content = content.replace(new RegExp(`(\\| ${dept}\\s+\\|).*?\\|.*?\\|.*?\\|`), `$1 ${status.padEnd(11)} | ${agent.padEnd(13)} | ${timestamp} |`);
